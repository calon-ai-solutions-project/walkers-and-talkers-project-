-- Also return the member's phone for the public tap card (per explicit request).
-- NOTE: this means a found/lost card, when tapped, will display the member's
-- phone number to whoever holds it. Switch back by removing 'phone' here.
...
begin
  ...
  return jsonb_build_object('status', v_result, 'first_name', v_member.first_name,
    'last_name', v_member.last_name, 'member_no', v_member.member_no, 'phone', v_member.phone,
    'region', v_region_name, 'walk_day', v_walk_day);
end; $$;
